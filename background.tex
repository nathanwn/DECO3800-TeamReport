...
\begin{table}[H]
  ...
  % \begin{enumerate}[1.]
  %   \item \textbf{Surveys}
  %     \par Survey is a method of quantitative research. It is used as an approach for collecting a more structured and statistical data for general background study. The participants were from various age groups as there are no conditional user restrictions, the possible solution is to cater to all age groups.  Thus, collected responses are used for further evaluation to create a solution for general users. Moreover, this is a crucial process to understand the requirements for system visualization during design planning. The survey was designed through Google Form and made available online. The responses are attached below in Appendix \ref{appendix:quantitative}.
  %     \par \textbf{Findings}
  %       \begin{figure}[H]
  %         \centering
  ...
  %         \item Based on Figure \ref{fig:quantitative-02}, 60\% of the respondents think that a contact-tracing application would be useful.
